...
class PseudoShareTracker:
    ...
    def track(self, wb, logfile):
        ...
        def _(share):#, timestamp, pseudo_share_count):
            ...
            logfile.write('%.6f share share:%s diff:%.8f diff:%.8f a:%d n:%d b:%d\n' % (
                timestamp, p2pool_data.format_hash(share.hash), share_dflty, block_dflty, self.count_accepted.count, self.count_in_my_share.count, self.count_in_block.count))
            self.count_in_current.deactivate()

            if share.hash in wb.my_share_hashes:
                #age = timestamp - my_share_data.start
                self.count_in_my_share.count += self.count_in_current.count
            ...
            if share.pow_hash <= share.header['bits'].target:
                ...
                self.count_in_block.count += self.count_in_current.count
